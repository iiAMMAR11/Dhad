...
def validate(document: dict[str, Any], resolver: Resolver) -> list[str]:
    ...
    for path in ("primitive.font.family.sans", "primitive.font.family.display"):
        if resolver.resolve(path)[0] != "IBM Plex Sans Arabic":
            errors.append(f"{path}: IBM Plex Sans Arabic must be the first family in every stack")
    ...
    for theme in ("dark", "light"):
        ...
        for status in ("writing", "ready", "editing", "done"):
            contrast_pairs.append((f"{theme} workflow {status}", f"recipe.production.color.{theme}.workflow.{status}", canvas, 4.5))
...
GLOBAL_CSS = path_map(
    ...
    *((f"font-size-{step}", f"primitive.font.size.{step}") for step in ("100", "200", "300", "400", "500", "600", "700", "800", "900", "1000")),
    ("font-size-teleprompter", "primitive.font.size.teleprompter"),
...
def theme_css_map(theme: str) -> OrderedDict[str, str]:
    ...
    production = f"recipe.production.color.{theme}.workflow"
    ...
    return path_map(
        ...
        ("status-writing", f"{production}.writing"),
        ("status-ready", f"{production}.ready"),
        ("status-editing", f"{production}.editing"),
        ("status-done", f"{production}.done"),
        ("production-status-writing", f"{production}.writing"),
        ("production-status-ready", f"{production}.ready"),
        ("production-status-editing", f"{production}.editing"),
        ("production-status-done", f"{production}.done"),
...
def render_css(document: dict[str, Any], resolver: Resolver) -> str:
    ...
    lines = [
        ...
        "   Do not edit this file directly. RTL first; dark is the default theme.",
        "   IBM Plex Sans Arabic (OFL-1.1) is the skill typeface; weights are bundled in assets/starter/fonts. */",
        "",
        ":root {",
    ]
    ...
    for role in ("micro", "caption", "label", "body", "bodyStrong", "title", "heading", "display", "timecode", "teleprompter"):
    ...
    for theme in ("dark", "light"):
        selector = ':root,\n[data-dhad-theme="dark"]' if theme == "dark" else '[data-dhad-theme="light"]'
...
NATIVE_COLOR_MAP = path_map(
    ...
    ("productionWorkflowWriting", "recipe.production.color.{theme}.workflow.writing"),
    ("productionWorkflowReady", "recipe.production.color.{theme}.workflow.ready"),
    ("productionWorkflowEditing", "recipe.production.color.{theme}.workflow.editing"),
    ("productionWorkflowDone", "recipe.production.color.{theme}.workflow.done"),
...
def render_flutter(document: dict[str, Any], resolver: Resolver) -> str:
    ...
    for name in ("100", "200", "300", "400", "500", "600", "700", "800", "900", "1000", "teleprompter"):
